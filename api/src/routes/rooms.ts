...
import { upsertLobbyRoom, markLobbyRoomGone, touchLobbyRoom } from '../db/lobby';
...
router.post('/lobby', async (req, res) => {
    ...
    upsertLobbyRoom(newRoom).catch(e => console.error(`[lobby] failed to persist room ${id}: ${e.message}`));
    notifyDiscord(`🆕 Room **${newRoom.name}** created by ${newRoom.owner} (${newRoom.players.length}/${newRoom.maxPlayers} players)`);
    res.json(newRoom);
});
...
router.post('/lobby/:id', async (req, res) => {
    ...
    console.log(`Lobby ping: ${room.name} [${id}] (${room.players.length} players)`);
    res.setHeader('Content-Type', 'application/json');
    res.json(room);
});
