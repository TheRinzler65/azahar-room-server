...
import { listActivityDB, listPlayerPresence, listGamePresence } from '../db/stats';
...
router.get('/stats/top-games', async (req, res) => {
    ...
    res.json(rows.map(r => ({ game: r.game, minutes: Math.round(Number(r.minutes)) })));
});

router.get('/stats/top-players', async (req, res) => {
    const rows = await listPlayerPresence(10);
    res.json(rows.map(r => ({ nickname: r.nickname, minutes: Math.round(Number(r.minutes)) })));
});
