...
export async function initDB() {
  ...
  try {
    await db.query(`ALTER TABLE lobby_rooms MODIFY id VARCHAR(36)`);
  } catch {}
  try {
    await db.query(`ALTER TABLE chat_messages MODIFY room_id VARCHAR(36)`);
  } catch {}
  try {
    await db.query(
      `ALTER TABLE room_instances MODIFY announced_room_id VARCHAR(64)`,
    );
  } catch {}
  try {
    await db.query(`ALTER TABLE bans ADD COLUMN expires_at BIGINT NULL`);
  } catch {}
...
export async function deleteReports(target: string) {
  const db = getPool();
  await db.query("DELETE FROM reports WHERE LOWER(target) = LOWER(?)", [
    target,
  ]);
}
