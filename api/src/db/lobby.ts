...
export async function upsertLobbyRoom(room: {
    id: string; name: string; owner?: string; port: number; maxPlayers: number;
    preferredGameName?: string; preferredGameId?: number; address?: string; hasPassword?: boolean;
}) {
    const now = Date.now();
    await pool.query(
        ...
        [room.id, room.name, room.owner || null, room.port, room.maxPlayers,
         room.preferredGameName || null, room.preferredGameId ?? null, room.address || null, room.hasPassword ? 1 : 0, now, now]
    );
}

export async function markLobbyRoomGone(id: string) {
    await pool.query(`UPDATE lobby_rooms SET status = 'gone' WHERE id = ? AND status = 'live'`, [id]);
}

export async function touchLobbyRoom(id: string) {
    await pool.query(`UPDATE lobby_rooms SET last_seen = ? WHERE id = ?`, [Date.now(), id]);
}

export async function expireLobbyRooms(maxAgeMs: number) {
    const cutoff = Date.now() - maxAgeMs;
    const [r] = await pool.query<RowDataPacket[]>(`SELECT id FROM lobby_rooms WHERE status = 'live' AND last_seen < ?`, [cutoff]);
    const ids = (r as unknown as { id: string }[]).map(x => x.id);
    if (ids.length) {
        await pool.query(`UPDATE lobby_rooms SET status = 'gone' WHERE id IN (?) AND status = 'live'`, [ids]);
    }
    return ids;
}

export async function listLobbyRooms(status?: 'live' | 'gone'): Promise<LobbyRoomRow[]> {
    const q = status
