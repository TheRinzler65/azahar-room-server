...
export function signPlayerJWT(username: string): string {
    return jwt.sign({ sub: username, role: 'player' }, privateKey, {
        algorithm: 'RS256', issuer: 'azahar-player', expiresIn: '24h'
    });
}
...
export function signAdminJWT(): string {
    ...
    return jwt.sign(
        { role: 'admin' },
        privateKey,
        { algorithm: 'RS256', issuer: 'azahar-admin', expiresIn: expiry as any }
    );
}
...
export function isAdmin(req: express.Request): boolean {
    ...
    try {
        const decoded = jwt.verify(authHeader.slice(7), publicKey, { algorithms: ['RS256'], issuer: 'azahar-admin' }) as any;
