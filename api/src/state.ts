import { recordActivityDB, addPlayerPresence, addGamePresence } from './db/stats';
...
export let chatLogs: { [roomId: string]: { username: string; message: string; timestamp: number }[] } = {};
...
export function setRooms(val: Room[]) { rooms = val; }
...
export function recordActivity(room: Room) {
    ...
    gamePresence[game] = (gamePresence[game] || 0) + (count > 0 ? 0.25 * count : 0);
