...
export const AdminAudit = () => {
    ...
    const getActionBadge = (action: string) => {
        if (action.startsWith('BAN_ADD')) {
            return <span className="bg-red-950/80 text-red-400 border border-red-800 px-1.5 py-0.5 rounded text-[10px] font-bold">BAN</span>;
        }
        if (action.startsWith('BAN_REMOVE')) {
            return <span className="bg-emerald-950/80 text-emerald-400 border border-emerald-800 px-1.5 py-0.5 rounded text-[10px] font-bold">UNBAN</span>;
        }
        if (action.includes('LOGIN')) {
            return <span className="bg-sky-950/80 text-sky-400 border border-sky-800 px-1.5 py-0.5 rounded text-[10px] font-bold">AUTH</span>;
        }
        if (action.startsWith('ROOM_')) {
            return <span className="bg-amber-950/80 text-amber-400 border border-amber-800 px-1.5 py-0.5 rounded text-[10px] font-bold">{action.replace('ROOM_', '')}</span>;
        }
        return <span className="bg-neutral-800 text-neutral-300 px-1.5 py-0.5 rounded text-[10px] font-bold">{action}</span>;
    };
    ...
    const filteredLogs = logs.filter(log => {
        ...
        return (
            ...
            (log.admin_username && log.admin_username.toLowerCase().includes(query)) ||
                ...
                <div className="border border-border overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody>
                            {loading ? (
                                <tr>
                                    <td colSpan={5} className="p-4 text-center text-neutral-500">Loading audit logs...</td>
                                </tr>
                            ) : filteredLogs.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="p-4 text-center text-neutral-500">No logs found</td>
                                </tr>
                            ) : (
                                filteredLogs.map((log, idx) => (
                                    <tr key={log.id || idx} className={`${idx % 2 === 0 ? 'bg-neutral-900' : 'bg-neutral-950'} hover:bg-neutral-800/40 border-b border-border/40`}>
