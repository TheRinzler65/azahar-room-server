...
export const ProfilePage = () => {
    ...
    if (!profile) return <div className="p-4 font-mono text-xs text-neutral-500">{error || 'Loading...'}</div>;
            ...
            <Window title={`PROFILE: ${profile.username.toUpperCase()}`}>
                <div className="font-mono text-xs space-y-2">
                    ...
                    <div className="flex justify-between border-b border-border pb-2">
                        ...
                        <span className="text-neutral-300 break-all">{profile.citraToken ? profile.citraToken : 'not set'}</span>
                    </div>
                    <div className="flex justify-between border-b border-border pb-2">
                        <span className="text-neutral-500">member since</span>
                        <span className="text-neutral-300">{new Date(profile.createdAt).toLocaleDateString()}</span>
                    </div>
                    <div className="flex justify-between border-b border-border pb-2">
                        <span className="text-neutral-500">playtime</span>
                        <span className="text-green-500">{hours}h {mins}min</span>
                    </div>
                    <div className="pt-4">
                        <button
                            onClick={logout}
