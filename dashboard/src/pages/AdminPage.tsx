...
export const AdminPage = () => {
    ...
    const [loggedIn, setLoggedIn] = useState(!!sessionStorage.getItem('azahar_admin_jwt'));
            ...
            {notifications.length > 0 && (
                <div className="border border-orange-800 bg-orange-950/30 font-mono text-xs">
                    <div className="flex items-center justify-between px-3 py-2 border-b border-orange-800/50">
                        <span className="text-orange-400 font-bold">NOTIFICATIONS ({notifications.length})</span>
                        <button onClick={clear} className="text-neutral-500 hover:text-neutral-300">[clear all]</button>
                    </div>
                    <div className="p-2 space-y-1 max-h-40 overflow-y-auto">
                        {notifications.map(n => (
                            <div key={n.id} className="flex justify-between items-center gap-2 text-neutral-300">
                                <div><span className="text-neutral-600">[{new Date(n.timestamp).toLocaleTimeString()}]</span> {n.message}</div>
                                <button onClick={() => dismiss(n.id)} className="text-neutral-600 hover:text-neutral-300">[x]</button>
                            </div>
                        ))}
                    </div>
                </div>
